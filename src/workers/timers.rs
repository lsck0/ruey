...
pub fn worker_start_timers(ui_diff_tx: mpsc::Sender<AppStateDiff>) {
    ...
    let ui_diff_tx_1 = ui_diff_tx.clone();
    tokio::spawn(async move {
        loop {
            ui_diff_tx_1.send(AppStateDiff::SaveSettings).unwrap();
            tokio::time::sleep(Duration::from_secs(30)).await;
        }
    });

    // check internet connection every 15 seconds
    let ui_diff_tx_2 = ui_diff_tx.clone();
    tokio::spawn(async move {
        ...
        loop {
            if TcpStream::connect_timeout(&cloudflare_dns_addr, Duration::from_secs(3)).is_ok() {
                ui_diff_tx_2.send(AppStateDiff::InternetConnected).unwrap();
            } else {
                ui_diff_tx_2.send(AppStateDiff::InternetDisconnected).unwrap();
            }
